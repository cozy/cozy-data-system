...
module.exports = Feed = (function() {
  ...
  function Feed(app) {
    ...
    this.logger = this.app.compound.logger;
    this.app.compound.server.on('close', function() {
      _this.stopListening();
  ...
  Feed.prototype.startPublishingToAxon = function(attempt) {
    var axon;
    ...
    return this.axonSock.sock.on('connect', function() {
      return console.info("An application conected to the change feeds");
    });
  };
  ...
  Feed.prototype._onChange = function(change) {
    ...
    if (change.deleted) {
      if (!deleted_ids[change.id]) {
        ...
        return this.db.post(doc, function(err, doc) {
      ...
      return this.db.get(change.id, function(err, doc) {
        var doctype, _ref;
