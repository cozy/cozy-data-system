...
var Account, Client, CryptoTools, User, accountManager, checkDocType, checkProxyHome, correctWitness, cryptoTools, db, encryptPassword, initPassword, keys, randomString, toString, user,
  _this = this;
  ...
  return user.getUser(function(err, user) {
    if (err) {
      ...
      if ((user.salt != null) && (user.slaveKey != null)) {
        return keys.logIn(body.password, user, function(err) {
          var _this = this;
          if (err != null) {
            send({
              error: err
        ...
        return keys.init(body.password, user, function(err) {
          if (err) {
            return send({
              error: err
            }, 500);
  ...
  if (body.password != null) {
    return user.getUser(function(err, user) {
      if (err) {
        ...
        return keys.update(body.password, user, function(err) {
          if (err) {
            return send({
              error: err
            }, 500);
...
action('resetKeys', function() {
  ...
  return user.getUser(function(err, user) {
    if (err) {
      ...
      console.log('reset');
      return keys.reset(user, function(err) {
        if (err) {
          return send({
            error: err
          }, 500);
...
action('deleteKeys', function() {
  return keys.logOut(function(err) {
    if (err) {
      return send({
        error: err
      }, 500);
