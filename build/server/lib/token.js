...
var addAccess, checkToken, db, defaultPermissions, fs, initAccess, initHomeProxy, permissions, productionOrTest, tokens, updatePermissions;
  ...
  if (productionOrTest) {
    return db.view('access/byApp', {
      ...
      if (accesses.length > 0) {
        ...
        delete permissions[access.login];
        delete tokens[access.login];
        return db.remove(access._id, access._rev, callback);
      } else {
        if (callback != null) {
          return callback();
        }
