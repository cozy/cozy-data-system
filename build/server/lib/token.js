...
var addAccess, checkToken, db, fs, initAccess, initHomeProxy, log, permissions, productionOrTest, ref, sharingPermissions, sharingTokens, tokens, updatePermissions;
...
module.exports.checkSharingRule = function(auth, docInfo, callback) {
  var err, i, isAuthenticated, len, name, ref1, ref2, ref3, rule;
  if (productionOrTest) {
    ref1 = checkToken(auth, sharingTokens), err = ref1[0], isAuthenticated = ref1[1], name = ref1[2];
    if (isAuthenticated) {
      if (((docInfo != null ? docInfo.id : void 0) != null) && ((docInfo != null ? docInfo.docType : void 0) != null)) {
        docInfo.docType = docInfo.docType.toLowerCase();
        ref2 = sharingPermissions[name];
        for (i = 0, len = ref2.length; i < len; i++) {
          rule = ref2[i];
          if (rule.id === docInfo.id && rule.docType === docInfo.docType) {
            return callback(null, name, true);
          }
        }
        return callback(null, name, false);
    ...
    ref3 = checkToken(auth, sharingTokens), err = ref3[0], isAuthenticated = ref3[1], name = ref3[2];
    if (name == null) {
...
module.exports.checkSharingRuleSync = function(auth, docInfo) {
  var err, i, isAuthenticated, len, name, ref1, ref2, ref3, rule;
  if (productionOrTest) {
    ref1 = checkToken(auth, sharingTokens), err = ref1[0], isAuthenticated = ref1[1], name = ref1[2];
    if (isAuthenticated) {
