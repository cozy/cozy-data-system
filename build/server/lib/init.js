...
var async, audio, db, defaultPermissions, getLostBinaries, initTokens, log, permissionsManager, thumb;
...
exports.addAudioMeta = function(callback) {
  return db.view('file/withoutAudioMeta', function(err, files) {
    if (err) {
      return callback(err);
    } else if (files.length === 0) {
      return callback();
